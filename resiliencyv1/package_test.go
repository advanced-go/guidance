...
const (
	docsAuthority = "github/advanced-go/documents"
)

func match(item any, values url.Values) bool {
...
var (
	...
	rsc = NewResource[Entry](docsAuthority, match, patch)
)

func init() {
	ctrl := controller.NewController("resiliencyv1", controller.NewPrimaryResource("", docsAuthority, time.Second*2, "", rsc.Do), nil)
